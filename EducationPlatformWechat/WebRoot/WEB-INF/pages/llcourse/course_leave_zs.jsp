	...
	<div class="weui_tab" style="width: 100%;">
    	...
    	<div class="weui_tab_bd" >
	        <div class="main_tabstyle" style="margin:5px 5px 5px;"> 
		   		<input placeholder="请选择日期" class="form-control" value="${nowdate }" name="shkdate" id="shkdate" lay-verify="shkdate" type="text" style="width: 60%;" readonly="readonly">
...
<script type="text/javascript">
	...
	function editxwqs(value){
	    ...
	    window.location.href="JxCourseArrange.do?method=toCourseLeaveInfo&xh="+value+"&openid="+openid+"&clientsxh="+clientsxh; 
	};
		...
		$.ajax({
			url:"JxCourseArrange.do?method=findProjectbyBj&stuxh="+stuxh+"&shkdate="+shkdate,
		  	...
		  	success: function(data){
		  		var list = eval(data);
		  		if(list.length==0){
		  			var html="<div style='width:100%;height:300px;'>";
		  				html+="</div>";
				...
				for(var i=0;i<list.length;i++){
		  			var html="<div class='weui_cells weui_cells_access' id='"+list[i].xh+"' onclick='editxwqs(this.id);'>";
		  				...
		  				html+="<p style='width: 85%'>"+list[i].coursename+"-"+list[i].rkjshname+"（"+list[i].skdate.substring(0,10)+"）</p>";
		  				html+="</div>";
			  			html+="<div class='weui_cell_ft' style='font-size:0.8em;'>点击请假</div>";
		  				html+="</a></div>";
		  				$("#task_div").append(html);
		  		}
