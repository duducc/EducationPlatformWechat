...
<script src="js/umgo-picker.js"></script>
 ...
 <script type="text/javascript">
 	...
 	function getDataList(){
 		...
 		$.ajax({
			url:"xfStudyRegistration.do?method=getMycourseLictInfo&year="+year+"&courseName="+courseName+"&usernumber="+usernumber,
		  	cache: false,
		  	async: true,  
		  	type:'post',
		  	dataType:'json',
 		...
 		<c:forEach items="${list }" var="p">
 				<div class="weui_cells weui_cells_access" onclick="editxwqs('${p.courseId}','${p.yitongCard}')">
      				<a class="weui_cell" href="javascript:;">
          				...
          				<c:if test="${p.state==2 }">
          					<div class="weui_cell_ft" style="font-size:0.8em;">取消报名</div>
          				</c:if>
          				<c:if test="${p.state==3 }">
          					<div class="weui_cell_ft" style="font-size:0.8em;">已签到</div>
          				</c:if>
          				<c:if test="${p.state==4 }">
 	...
 	*/
 	...
 	function editxwqs(value){
 	    ...
 	    var usernumber = $("#usernumber").val(); 
 	    if(type=="0"){
		    window.location.href="xfStudyRegistration.do?method=selectXfCourseByPrimaryKey&courseId="+courseId+"&isgrant="+isgrant+"&awardstate="+awardstate+"&remarks3="+remarks3+"&openId="+openId+"&clientsxh="+clientsxh; 
 	    }else if(type=="1"){
 	    	if(remarks3=="1"){
 	    		$.alert("该课题已反馈！","提示");
 	    		return;
 	    	}else{
	 	    	window.location.href="xfStudyRegistration.do?method=toTaskInfoTickling&courseId="+courseId+"&usernumber="+usernumber+"&xh="+xh+"&openId="+openId+"&clientsxh="+clientsxh;
 	    	}
 	    }
	}
