...
public class UploadHandleServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
                ...
                try{
                    ...
                    for(FileItem item : list){
                        ...
                        if(item.isFormField()){
                            ...
                            String saveFilenameStr = new String(saveFilename.getBytes("ISO-8859-1"),"UTF-8");
                            //得到文件的保存目录
                ...
                //request.getRequestDispatcher("/WEB-INF/pages/systemManager/frm_issue_upload_success.jsp").forward(request, response);
                //response.setCharacterEncoding("text/html,charset=utf-8");
                resout=response.getWriter();
                List<String> result = new ArrayList<String>();
                //String content = new String(entity.getContent().getBytes("ISO-8859-1"), "GBK");
                message = new String(message.getBytes("ISO-8859-1"),"utf-8");
                String pathAndFileName = returnPathAndFileName();
                System.out.println("文件名1:"+pathAndFileName);
               // pathAndFileName = new String(pathAndFileName.getBytes("ISO-8859-1"),"utf-8");
                byte[] b = pathAndFileName.getBytes("utf-8");//编码  
                String transFileName = new String(b, "gbk");//解码:
                System.out.println("转码文件名:"+pathAndFileName);
                result.add(transFileName);
                //result.add(message);
                resout.write(new Gson().toJson(result));
    }
